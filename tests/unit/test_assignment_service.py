...
@pytest.fixture
def service():
    return AssignmentService(CONDITIONS)


class TestAssignmentService:
    def test_returns_a_condition(self, service):
        condition = service.assign("P-AAAAAAAA")
        assert condition["id"] in {"A", "B"}

    def test_deterministic(self, service):
        c1 = service.assign("P-AAAAAAAA")
        c2 = service.assign("P-AAAAAAAA")
        assert c1["id"] == c2["id"]
    ...
    def test_sha256_matches_manual(self, service):
        ...
        assert service.assign(pid)["id"] == expected_id

    def test_validate_correct(self, service):
        pid = "P-AAAAAAAA"
        condition = service.assign(pid)
        assert service.validate(pid, condition["id"]) is True

    def test_validate_incorrect(self, service):
        pid = "P-AAAAAAAA"
        condition = service.assign(pid)
        wrong_id = "B" if condition["id"] == "A" else "A"
        assert service.validate(pid, wrong_id) is False
